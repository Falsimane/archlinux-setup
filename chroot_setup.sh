...
sed -i 's/^MODULES=.*/MODULES=(vmw_balloon vmw_pvscsi vsock vmw_vsock_vmci_transport vmw_vmci vmwgfx vmxnet3)/' /etc/mkinitcpio.conf
ok "Hooks + modules VMware mkinitcpio configurés"
...
# ============================================================================
# VMWARE GUEST — Configuration complète
# ============================================================================
step "VMware — Configuration Guest"

# user_allow_other pour vmhgfs-fuse shared folders
sed -i 's/^#user_allow_other/user_allow_other/' /etc/fuse.conf
ok "fuse.conf : user_allow_other activé"

# Time sync via host
vmware-toolbox-cmd timesync enable 2>/dev/null || warn "vmware-toolbox-cmd non disponible (normal en chroot)"
ok "Synchronisation temps VMware configurée"

# fstab entry pour shared folders (commentée — à adapter avec le nom du partage)
cat <<'VMFSTAB' >> /etc/fstab

# VMware Shared Folders (décommentez et adaptez le nom du partage)
#.host:/  /mnt/hgfs  fuse.vmhgfs-fuse  nofail,allow_other  0  0
VMFSTAB
mkdir -p /mnt/hgfs
ok "Point de montage /mnt/hgfs prêt (fstab commenté — à adapter)"
